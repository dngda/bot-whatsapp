...
module.exports = HandleMsg = async (client, message) => {
    try {
        ...
        if (isCmd && !isGroupMsg) { console.log(color('[EXEC]'), color(moment(t * 1000).format('DD/MM/YY HH:mm:ss'), 'yellow'), color(`${command} [${args.length}] : ${arg.substring(10, 0)}...`), 'from', color(pushname)) }
        if (isCmd && isGroupMsg) { console.log(color('[EXEC]'), color(moment(t * 1000).format('DD/MM/YY HH:mm:ss'), 'yellow'), color(`${command} [${args.length}] : ${arg.substring(10, 0)}...`), 'from', color(pushname), 'in', color(name || formattedTitle)) }

...
const createList = (groupId, listName, desc) => new Promise((resolve, reject) => {
	try {
		...
		if (find && find.id === groupId) {
			const getList = db.chain.get('groups').filter({ id: groupId }).map('list').value()[0]
			const isIn = inArray(listName, getList, 'name')
			if (isIn !== false) {
				resolve(false)
			} else {
				getList.push({ name: listName, desc: desc, listData: [] })
				db.chain.get('groups').find({ id: groupId }).set('list', getList).value()
				db.write()
				resolve(true)
			}
		} else {
			db.chain.get('groups').push({ id: groupId, list: [{ name: listName, desc: desc, listData: [] }] }).value()
			db.write()
			resolve(true)
		}
	} catch (err) {
...
const ListData = (groupId, listName, newData, index, action) => new Promise((resolve, reject) => {
	try {
		...
		const isIn = inArray(listName, getList, 'name')
		if (isIn !== false) {
			let data = db.chain.get('groups').filter({ id: groupId }).map(`list[${isIn}]`)

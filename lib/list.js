 ...
 */
const ListData = async (groupId, listName, listContentDataOrIndex, isRemove) => new Promise((resolve, reject) => {
	try {
		const getList = db.get('group').filter({ id: groupId }).map('list').value()[0]
		const isIn = inArray(listName, getList, 'name')
		if (isIn !== false) {
 ...
 */
const addListData = async (groupId, listName, listContentData) => new Promise((resolve, reject) => {

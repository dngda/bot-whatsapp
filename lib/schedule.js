...
const futureMilis = (client, msg, content, milis, isQuoted) => new Promise(async (resolve, reject) => {
	...
	const job = schedule.scheduleJob(when, function(n) {
		if (!isQuoted) {
			if (n.includes('@')) client.sendTextWithMentions(msg.from, n)
				else client.sendText(msg.from, n)
...
const loadJob = (client, from, quotedId, content, date, isQuoted) => new Promise(async (resolve, reject) => {
	...
	const job = schedule.scheduleJob(dateJob, function(n) {
		if (!isQuoted) { 
			if (n.includes('@')) client.sendTextWithMentions(from, n)
				else client.sendText(from, n)

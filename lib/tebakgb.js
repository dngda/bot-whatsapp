	...
	try {
		scrapeTebakGambar().then((res) => {
			...
			if (find && find.id === groupId) {
				db.chain.get('groups').find({ id: groupId }).set('ans', ans).value()
				db.write()
				resolve(res)
			} else {
				db.chain.get('groups').push({ id: groupId, ans: ans }).value()
				db.write()
				resolve(res)
			}
		}).catch((err) => {
...
const getAns = (groupId) => new Promise((resolve, reject) => {
	try {
		const find = db.chain.get('groups').find({ id: groupId }).value()
		if (find && find.id === groupId) {
			const ans = db.chain.get('groups').find({ id: groupId }).value()
			resolve(ans)
		} else {
			resolve(false)
		}
...
const delData = (groupId) => new Promise((resolve, reject) => {
	try {
		const res = db.chain.get('groups').remove({ id: groupId }).value()
		db.write()

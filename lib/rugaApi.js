...
const dewabatch = async (judul) => new Promise((resolve, reject) => {
	axios.get(`https://arugaz.my.id/api/anime/kusonime/search?query=${judul}`)
	.then((res) => {
...
const ytmp4 = async (url) => new Promise((resolve, reject) => {
    axios.get(`https://arugaz.my.id/api/media/ytvid?url=${url}`)
    .then((res) => {
		resolve(res.data)
    })
    .catch((err) =>{
...
const fb = async (url) => new Promise((resolve, reject) => {
	axios.get(`http://arugaz.my.id/api/media/facebook?url=${url}`)
	.then((res) => {
...
const stalkig = async (url) => new Promise((resolve, reject) => {
    axios.get(`http://arugaz.my.id/api/media/stalkig?user=${url}`)
    .then((res) => {
...
const quote = async () => new Promise((resolve, reject) => {
    axios.get(`https://arugaz.my.id/api/random/text/quotes`)
    .then((res) => {
        const text = `Author: ${res.data.result.by}\n\nQuote: ${res.data.result.quote}`
        resolve(text)
    })
    .catch((err) =>{
        reject(err)
...
const tulis = async (teks) => new Promise((resolve, reject) => {
    axios.get(`https://tools.zone-xsec.com/api/nulis.php?q=${encodeURIComponent(teks)}`)
    .then((res) => {
...
const artinama = async (nama) => new Promise((resolve, reject) => {
	axios.get(`https://arugaz.my.id/api/primbon/artinama?name=${nama}`)
	.then((res) => {
		resolve(res.data.result)
	})
	.catch((err) => {
...
const cekjodoh = async (nama, lahir1, pasangan, lahir2) => new Promise((resolve, reject) => {
	axios.get(`https://arugaz.my.id/api/primbon/ramalanjodoh?nama1=${nama}&tgl1=${lahir1}&nama2=${pasangan}&tgl2=${lahir2}`)
	.then((res) => {
...
const movie = async (judul) => new Promise((resolve, reject) => {
	axios.get(`http://arugaz.my.id/api/media/filmapik/search?query=${judul}`)
	.then((res) => {
		if (res.data.error) return resolve({status: 'error', hasil: res.data.result})
		const teksmov = `Judul: ${res.data.title}\nRating: ${res.data.rating}\nLink: ${res.data.link}`
		resolve({status: 'success', hasil: teksmov, link: res.data.thumb})
	})
	.catch((err) => {
		reject(err)
	})

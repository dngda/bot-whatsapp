...
const { apiTobz, apiVH, apiFarzain, apiItech } = JSON.parse(readFileSync('./settings/api.json'))

const quote = () => new Promise((resolve, reject) => {
    slash = sample(["quotes", "quotes2", "quotes3"])
...
const ytsearch = (query) => new Promise((resolve, reject) => {
    get(`https://videfikri.com/api/ytsearch/?query=${encodeURIComponent(query)}`)
        .then((res) => {
...
const artinama = (nama) => new Promise((resolve, reject) => {
    get(`http://zekais-api.herokuapp.com/artinama?nama=${encodeURIComponent(nama)}`)
        .then((res) => {

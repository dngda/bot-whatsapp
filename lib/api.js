...
const tulis = (teks) => new Promise((resolve, reject) => {
    axios.get(`https://videfikri.com/api/nulis/?query=${encodeURIComponent(teks)}`)
    .then((res) => {
...
const tiktod = (url) => new Promise((resolve, reject) => {
    axios.get(`https://videfikri.com/api/tiktok/?url=${url}`)
       .then(res => {
        resolve(res.data.result)
    }).catch(e => reject(e))
})

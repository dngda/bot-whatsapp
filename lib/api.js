...
const { apiFarzain, apiItech, apiZeks, apiZenz } = JSON.parse(readFileSync('./settings/api.json'))
 ...
 *
 * @param  {String} query
 * @returns {object} { thumb, hd }
 */
const fbdl = (url) => new Promise((resolve, reject) => {
    get(`https://zenzapi.xyz/api/downloader/facebook?url=${url}&apikey=${apiZenz}`)
        .then(res => {
 ...
 *
 * @param  {String} query
 * @returns {object} { sd, hd, fhd }
 */
const twdl = (url) => new Promise((resolve, reject) => {
    get(`https://zenzapi.xyz/api/downloader/twitter?url=${url}&apikey=${apiZenz}`)
        .then(res => {
            if (res.data.status !== true) reject(res.data.status)
            else resolve(res.data.result.media)
        })
        .catch(err => {
            reject(err)
        })
...
const igdl = (url) => new Promise((resolve, reject) => {
    get(`https://zenzapi.xyz/api/downloader/instagram2?url=${url}&apikey=${apiZenz}`)
        .then(res => {
            if (res.data.code !== 200) reject(res.data.code)
            else resolve(res.data.result)
        })
...
const simi = (inp) => new Promise((resolve, reject) => {
    get(`https://zenzapi.xyz/api/simih?text=${encodeURIComponent(inp)}&apikey=${apiZenz}`)
        .then(res => {
            if (res.data.status !== true) reject(res.data.status)
            else resolve(res.data.result.message)
        })
        .catch(err => {
            reject(err)
        })

 ...
 * @ Modified time: 2021-07-06 12:18:32
...
const pinterestLight = (querry) => new Promise((resolve, reject) => {
    ...
    axios.get(`https://id.pinterest.com/search/pins/?q=` + querry, {
        ...
        hasil.map(V => {
            ...
            Data.add(V.replace('236x', 'originals'))
        })
        ress = {
            status: res.status,
            author: "RA",
 ...
 * @returns {Promise} `Promise` that resolve obj
 */
const facebookSaveFrom = (browser, url) => new Promise(async (resolve, reject) => {
	try {
		const page = await browser.newPage()
		await page.goto("https://en.savefrom.net/9-how-to-download-facebook-video.html", { timeout: 10000, waitUntil: 'load' })

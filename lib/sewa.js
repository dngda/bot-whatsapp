...
const getListSewa = (client) => new Promise(async (resolve, reject) => {
    try {
        db.read()
        let { groups } = db.data
        let _groups = groups.map(async (group) => {
...
const getExp = (id) => new Promise((resolve, reject) => {
    try {
        db.read()
        const find = db.chain.get('groups').find({ groupId: id }).value()
        if (find && find.groupId === id) var msg = new Date(find.expire).toLocaleDateString('id-ID', { weekday: 'long', year: 'numeric', month: 'long', day: 'numeric' })
        else var msg = ''
        resolve(msg)
    } catch (e) {
        reject(e)

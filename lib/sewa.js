...
const listenSaweria = (client, browser) => new Promise(async (resolve, reject) => {
    try {
        ...
        await page.exposeFunction('puppeteerLogMutation', async () => {
            ...
            client.sendText(ownerNumber, `Cekringgg!! Donatur: ${who} Total: ${much} Pesan: ${msg}`)
...
const sewaBot = (client, linkGroup, duration) => new Promise(async (resolve, reject) => {
    try {
        ...
        if (find && find.groupId === groupInfo.id) {
            ...
            client.sendText(groupInfo.id, `Perpanjang Penyewaan Berhasil. Bot akan keluar sendiri pada tanggal ${expireAt.toString()}`)
            ...
            await client.joinGroupViaLink(linkGroup)
                .then(async () => {
                    resolve(true)
                    setTimeout(async () => {
                        await client.sendText(groupInfo.id, `Penyewaan Berhasil. Bot akan keluar sendiri pada tanggal ${expireAt.toString()}`)

...
const sewaBot = (client, linkGroup, duration) => new Promise(async (resolve, reject) => {
    try {
        ...
        if (find && find.groupId === groupInfo.id) {
            ...
            client.sendText(groupInfo.id, `Perpanjang Penyewaan Berhasil!\nBot akan keluar sendiri pada hari ${expireAt.toLocaleDateString('id-ID', localDateOptions)}`)
...
const checkExpireSewa = (client) => new Promise((resolve, reject) => {
    try {
        db.read()
        let now = new Date()
        let oneDayBeforeNow = new Date(Date.now() - 86400000)
        let { groups } = db.data
        groups.forEach(async (group) => {
            ...
            if (dateExpire <= now) {
                ...
                client.sendText(ownerNumber, `Penyewaan untuk group dengan:\n-> \`\`\`Id   :\`\`\` ${groupId}\n-> \`\`\`Name :\`\`\` ${info.title}\nTelah selesai.`)
                setTimeout(async () => {
            ...
            } else if (dateExpire <= oneDayBeforeNow) {
...
const getListSewa = (client) => new Promise(async (resolve, reject) => {
    try {
        ...
        groups = await groups.map(async (group) => {
            ...
            let expire = date.toLocaleDateString('id-ID', localDateOptions)
            return {
                title: title,
...
const getExp = (id) => new Promise((resolve, reject) => {
    try {
        ...
        if (find && find.groupId === id) {
            ...
            var msg = date.toLocaleDateString('id-ID', localDateOptions)
        } else {
            var msg = ''
        }
        resolve(msg)

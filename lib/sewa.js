...
const sewaBot = (client, linkGroup, duration) => new Promise(async (resolve, reject) => {
    try {
        ...
        if (find && find.groupId === groupInfo.id) {
            ...
            db.chain.get('groups').filter({ groupId: groupInfo.id }).update('expire', n => n = expireAt).value()
            db.write()
        } else {
            ...
            await client.joinGroupViaLink(linkGroup)
                .then(async () => {
                    resolve(true)
                    setTimeout(async () => {
                        await client.sendText(groupInfo.id, `Penyewaan Berhasil. Bot akan keluar sendiri pada hari ${expireAt.toLocaleDateString('id-ID', { weekday: 'long', year: 'numeric', month: 'long', day: 'numeric' })}`)
    ...
    try {
        ...
        groups.forEach((group) => {
            ...
            if (dateExpire <= now) {
                ...
                db.write()
            } else if (dateExpire <= oneDayBefore) {
...
const getListSewa = (client) => new Promise(async (resolve, reject) => {
    try {
        ...
        groups = await groups.map(async (group) => {
            ...
            let expire = date.toLocaleDateString('id-ID', { weekday: 'long', year: 'numeric', month: 'long', day: 'numeric' })
            return {
                title: title,
...
const getExp = (id) => new Promise((resolve, reject) => {
    try {
        ...
        if (find && find.groupId === id) {
            ...
            var msg = date.toLocaleDateString('id-ID', { weekday: 'long', year: 'numeric', month: 'long', day: 'numeric' })
        } else {
            var msg = ''
        }
        resolve(msg)
